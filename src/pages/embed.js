                ...
                <div className="container h-full flex flex-col">
                    ...
                    <div className="flex flex-col lg:flex-row h-full mt-10">
                        <div className="h-full w-full lg:w-3/5 flex flex-col justify-center gap-4 px-4 lg:px-0 ">
                            ...
                            <button className="btn bg-black text-white mt-5 w-fit">Get Started</button>
                        </div>
                        <div className="flex justify-center items-center mt-8 lg:mt-0 relative w-full md:w-1/2 h-full min-h-[400px] mx-auto">
                            <Image src="/assets/img/embedheroimage.svg" layout={'fill'} alt="Embed Image" />
                        </div>
                    </div>
                </div>
            </div>

            <div className="h-fit lg:h-screen w-full flex justify-center items-center py-10 md:py-20 gap-10">
                <div className="w-full lg:w-4/6 h-full px-4 md:px-0 flex flex-col md:flex-row justify-center items-center gap-4">
                    <div className="flex justify-center items-center relative w-full md:w-1/2 h-full min-h-[400px] mx-auto">
                        <Image src="/assets/img/dummyimg.svg" layout={'fill'} alt="Embed Image" />
                    </div>
                    <div className="w-fit h-fit py-10 px-0 flex flex-col justify-around items-center gap-16 p-10">
                        {connectorData.map((item, index) => (
                            <div
                                key={index}
                                className="relative pt-4 border-t-2 sub__h2 pr-6 lg:pr-0 pb-6 md:pb-0 text-gray-400"
                            >
                                <span className="absolute -top-4 left-0 bg-white pr-0 md:pr-2 text-black">
                                    {item.title}
                                </span>
                                {item.description}
                            </div>
